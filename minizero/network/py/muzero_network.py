...
from .network_unit import ResidualBlock, PolicyNetwork, ValueNetwork, SwitchNetwork
...
class MuZeroPredictionNetwork(nn.Module):
    def __init__(self, num_channels, channel_height, channel_width, action_size, num_value_hidden_channels):
        ...
        self.switch = SwitchNetwork(num_channels, channel_height, channel_width)

    def forward(self, hidden_state):
        policy_logit = self.policy(hidden_state)
        value = self.value(hidden_state)
        switch = self.switch(hidden_state)
        return policy_logit, value, switch
...
class MuZeroNetwork(nn.Module):
    ...
    def initial_inference(self, state):
        ...
        policy_logit, value, switch = self.prediction_network(hidden_state)
        policy = torch.softmax(policy_logit, dim=1)
        return {"policy_logit": policy_logit, "policy": policy, "value": value, "switch": switch, "hidden_state": hidden_state}
    ...
    def recurrent_inference(self, hidden_state, action_plane):
        ...
        policy_logit, value, switch = self.prediction_network(next_hidden_state)
        policy = torch.softmax(policy_logit, dim=1)
        return {"policy_logit": policy_logit, "policy": policy, "value": value, "switch": switch, "hidden_state": next_hidden_state}
