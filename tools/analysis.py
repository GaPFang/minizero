    ...
    for index in range(2):
        ...
        for line in lines[index]:
            ...
            if ret1 != []:
                ...
                continue
            ret4 = re.findall(r'(\[SelfPlay Avg. Legal Player Rates\])\s(-?\d+\.\d+|\d+)', line)
            if ret4 != []:
                key = ret4[0][0]
                if key not in myDict:
                    myDict[key] = []
...
def analysis_(dir, path, iter, all: bool = False, name: bool = False):
    ...
    Fig_list = list(set(["Lengths", "Time", "Returns", "Legal Player Rates"] + [re.sub(r"_\d+$", "", key) for key in myDict if re.match(r'^(loss|accuracy)_', key)]))
    # plt figure
    ...
    for item in Fig_list:
        ...
        for key in myDict:
            if item in key:
                bool_print = True
                if "Returns" in item or "Lengths" in item or "Legal Player Rates" in item:
